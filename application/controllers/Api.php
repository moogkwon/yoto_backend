...
class Api extends CI_Controller
{
    ...
    public function uploadPhoto()
    {
        ...
        if (!$_FILES['photo']['tmp_name'] || !$_FILES['photo']['tmp_name'] || $_FILES['photo']['error']) {
            $result = ['code' => 400, 'message' => 'File upload error'];
            echo json_encode($result);
            return false;
        }
        ...
        // Store photo
        $input = S3::inputFile($_FILES['photo']['tmp_name']);
        $uriPhoto = 'profile/' . str_pad($user->id, 8, '0', STR_PAD_LEFT) . '-' . mt_rand(10000, 99999) . '.jpg';
        if (!S3::putObject($input, $this->s3Bucket, $uriPhoto, S3::ACL_PUBLIC_READ)) {
            $result = ['code' => 500, 'message' => 'Can`t store file to S3'];
            echo json_encode($result);
            return false;
        }
